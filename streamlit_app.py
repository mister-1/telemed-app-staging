# Telemedicine Dashboard — Modern Pastel + Dark + Robust Admin (v3)
        ...
        with st.form('login'):
            ...
            if st.form_submit_button('Login'):
                ...
                if rows and verify_pw(p, rows[0]['password_hash']):
                    st.session_state.auth={'ok':True,'user':rows[0]['username']}; rerun()
...
with st.sidebar:
    ...
    if nav != page:
        st.query_params.update({'page':nav})
        rerun()
...
def render_dashboard():
    ...
    # Filters
    c1,c2,c3,c4 = st.columns([1.4,1.4,1.8,1])
    with c1:
    ...
    with c2:
        # Multiselect โรงพยาบาล (ค้นหาได้) + ปุ่มเลือกทั้งหมด/ล้าง
        names = sorted(hospitals_df.get('name', pd.Series(dtype=str)).dropna().unique().tolist())
        default_all = st.session_state.get('hosp_sel', names)
        sel = st.multiselect('🏥 โรงพยาบาล', options=names, default=default_all, key='hosp_sel')
        cc1, cc2 = st.columns(2)
        with cc1:
            if st.button('เลือกทั้งหมด'): st.session_state.hosp_sel = names; rerun()
        with cc2:
            if st.button('ล้างทั้งหมด'): st.session_state.hosp_sel = []; rerun()
    with c3:
        # Site control multiselect
        sf_default = st.session_state.get('site_filter', SITE_CONTROL_CHOICES)
        st.session_state.site_filter = st.multiselect('🧭 ทีมภูมิภาค (SiteControl)', options=SITE_CONTROL_CHOICES, default=sf_default)
    with c4:
        st.caption(' ')
        if st.button('🖨️ บันทึกหน้า'): st.write("<script>window.print()</script>", unsafe_allow_html=True)
    ...
    # Horizontal bar by hospital (use plotly_chart to keep native fullscreen)
    ...
    if not daily.empty:
        ...
        labels = daily['date'].apply(lambda d: f"{TH_DAYS[pd.to_datetime(d).dayofweek]} {pd.to_datetime(d).day}/{pd.to_datetime(d).month}/{str(pd.to_datetime(d).year)[-2:]}")
        ln = go.Figure()
    ...
    try:
        st.table(
            site_tbl.style
                .set_table_styles([{'selector':'th','props':[('background','#334155' if DARK else '#EEF2FF'),
...
def render_admin():
    if not st.session_state.auth['ok']:
        st.warning('กรุณาเข้าสู่ระบบทาง Sidebar ก่อน')
        return
        ...
        with st.expander('➕ เพิ่ม/แก้ไข โรงพยาบาล'):
            ...
            with c1:
                if st.button('บันทึกโรงพยาบาล'):
                    ...
                    if edit_mode:
                        sb.table('hospitals').update(payload).eq('id', row['id']).execute()
    ...
    with tabs[1]:
        ...
        if hospitals_df.empty:
            ...
            if raw_tx.empty:
                ...
                st.dataframe(tx_view[safe_cols(tx_view, desired)].rename(
                    columns={'name':'โรงพยาบาล','date':'วันที่','transactions_count':'Transactions','riders_active':'Rider Active'}),
    ...
    with tabs[3]:
        ...
        with a:
            if st.button('เติมข้อมูลตัวอย่าง (5 รพ. x 3 วัน)'):
                ...
                st.success('เติมข้อมูลแล้ว'); load_df.clear(); rerun()
        with b:
